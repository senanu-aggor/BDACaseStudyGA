...
# Now you have your data completely clean to 
...
ss <- subset(gadata,pagePath=="/")

bs <- ddply(ss,.(pagePath,sourceMedium,userType,deviceCategory),summarise, entrances=sum(entrances),pageviews=sum(pageviews),exits=sum(exits),timeOnPage=sum(timeOnPage),
      uniquePageviews=sum(uniquePageviews),pageLoadTime=sum(uniquePageviews))
head(ts)

ss <- tbl_df(ss)
ss <- arrange(ss,sourceMedium,userType,deviceCategory)
as <- group_by(ss,pagePath,sourceMedium,userType,deviceCategory) 
bs <- (summarise(as,
    entrances=sum(entrances),pageviews=sum(pageviews),exits=sum(exits),timeOnPage=sum(timeOnPage),
    uniquePageviews=sum(uniquePageviews),pageLoadTime=sum(uniquePageviews)))
bs
ss %>% group_by(pagePath,sourceMedium,userType,deviceCategory) %>%
